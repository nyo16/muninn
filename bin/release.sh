...
run_tests() {
  ...
  if ! MUNINN_BUILD=true mix test; then
...
check_formatting() {
  ...
  if ! (cd native/muninn && cargo fmt -- --check); then
...
AFTER RELEASE:
  1. Wait for GitHub Actions to complete: https://github.com/nyo16/muninn/actions
  2. Download checksums: ./bin/checksum.sh
  3. Commit checksums: git add checksum-*.exs && git commit -m "Update checksums" && git push
...
if [[ "$DRY_RUN" == "false" ]]; then
  ...
  print_info "Check the Actions tab at: https://github.com/nyo16/muninn/actions"
  echo ""
  print_info "After builds complete, run:"
  echo "  ./bin/checksum.sh"
  echo "  git add checksum-*.exs && git commit -m 'Update checksums for v$VERSION' && git push"
